...
![Build & Release](https://github.com/kwonyoungchan/CommStack/actions/workflows/release.yml/badge.svg)
...
빌드 없이 바로 사용하려면 [Releases](https://github.com/kwonyoungchan/CommStack/releases)에서 최신 `CommStack-vX.X.X-win-x64.zip`을 다운로드하세요.
...
int main() {
    // 1. 수신 채널 생성 (포트 9000에서 수신 대기)
    void* hRecv = CreateCommunicationChannel(1); // 1 = UDP
    ChannelConfig recvCfg{};
    recvCfg.m_nChannelId    = 1;
    recvCfg.m_nLocalPort    = 9000;
    recvCfg.m_nTargetPort   = 9001;
    recvCfg.m_nBufferSize   = 1024;  // 반드시 2의 거듭제곱
    recvCfg.m_nCpuCoreIndex = 0;
    std::memcpy(recvCfg.m_szTargetIp, "127.0.0.1", 10);
    OpenCommunicationChannel(hRecv, &recvCfg);

    // 2. 송신 채널 생성 (9001 → 9000으로 송신)
    void* hSend = CreateCommunicationChannel(1);
    ChannelConfig sendCfg{};
    sendCfg.m_nChannelId    = 2;
    sendCfg.m_nLocalPort    = 9001;
    sendCfg.m_nTargetPort   = 9000;
    sendCfg.m_nBufferSize   = 1024;
    sendCfg.m_nCpuCoreIndex = 1;
    std::memcpy(sendCfg.m_szTargetIp, "127.0.0.1", 10);
    OpenCommunicationChannel(hSend, &sendCfg);

    // 3. 데이터 송신
    const char* msg = "Hello, CommStack!";
    SendPacketData(hSend, reinterpret_cast<const uint8_t*>(msg),
                   static_cast<uint32_t>(std::strlen(msg) + 1));

    // 4. 수신 폴링 (논블로킹)
    std::this_thread::sleep_for(std::chrono::milliseconds(10));
    uint8_t buf[2048];
    uint32_t size = 0;
    if (ReceivePacketData(hRecv, buf, &size)) {
        std::printf("수신: %s (%u bytes)\n", reinterpret_cast<char*>(buf), size);
    }

    // 5. 정리
    DestroyCommunicationChannel(hSend);
    DestroyCommunicationChannel(hRecv);
    return 0;
}
```

### C# — ChannelManager 최소 예제
...
// 1. 채널 매니저 생성
using var manager = new ChannelManager();

// 2. UDP 채널 등록 (ID=1, 127.0.0.1:9000 수신, 9001 로컬)
manager.AddChannel(ChannelType.Udp, id: 1,
    ip: "127.0.0.1", remotePort: 9000, localPort: 9001, coreIndex: 0);

// 3. 데이터 송신
byte[] payload = System.Text.Encoding.UTF8.GetBytes("Hello, CommStack!");
manager.SendTo(channelId: 1, payload);

// 4. 수신 폴링 (시뮬레이터 메인 루프에서 매 틱 호출)
manager.UpdateReceive((channelId, data, size) =>
{
    // ⚠️ data 버퍼는 공유 버퍼이므로 반드시 복사해서 사용할 것
    byte[] copy = data[..(int)size].ToArray();
    Console.WriteLine($"[CH {channelId}] 수신: {copy.Length} bytes");
});
```

> **주의:** `BufferSize`는 반드시 **2의 거듭제곱**(512, 1024, 2048…)이어야 합니다.  
> C# `UpdateReceive` 콜백의 `data` 배열은 공유 버퍼이므로, 콜백 밖에서 사용하려면 반드시 복사하세요.
